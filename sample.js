...
var keyupStack = [];
$('#keyword').on('keyup', function () {//keyupはキーが抑れ上がった瞬間に事項される。
  keyupStack.push(1);
  ...
  setTimeout(function () {
    ...
    if (keyupStack.length === 0) {//
      ...
      var buf = '.*' + $(this).val().replace(/(.)/g, "$1.*");//今回のthisは#keyword。(.)は何かしらすべての文字。/gはここまでという意味？
      ...
      var filteredLists = $.grep(data, function (d) {//grepメソッドは、配列の中からフィルタ関数を利用して、特定の値だけを残した配列を返す。引数は第一に配列、第二は配列のインデックス
       return reg.test(d);
      }.bind(this));
...
var createRow = function (lists) {//listsはdata
  var list = $('#list');
  $('#list').empty();//書く子要素のすべてを削除し、空にする。イベントハンドラなどの情報も削除する。
